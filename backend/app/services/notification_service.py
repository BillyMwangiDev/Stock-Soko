...
from ..config import FIREBASE_CREDENTIALS_PATH, FIREBASE_PROJECT_ID, ENABLE_NOTIFICATIONS
...
class NotificationService:
    ...
    def send_multicast(
        ...
        if not self.initialized:
            logger.info(f"[MOCK] Multicast: {title} - {body} to {len(fcm_tokens)} devices")
...
def send_price_alert_notification(user: Any, symbol: str, alert_type: str, target_price: float, current_price: float):
    ...
    if alert_type == "above":
        ...
        body = f"{symbol} is now above KES {target_price:.2f} at KES {current_price:.2f}"
    elif alert_type == "below":
        title = f"Price Alert: {symbol}"
        body = f"{symbol} is now below KES {target_price:.2f} at KES {current_price:.2f}"
    else:
        title = f"Price Alert: {symbol}"
...
def send_trade_notification(user: Any, order_type: str, symbol: str, quantity: int, price: float):
