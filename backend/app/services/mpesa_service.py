...
from ..database.models import Account, Portfolio, Transaction, User
...
def initiate_mpesa_stk(
    req: MpesaDepositRequest, db: Session = None
    ...
    if req.account_id and db:
        ...
        if not account:
            ...
            return MpesaDepositResponse(
                checkout_request_id="", status="error", message="Invalid broker account"
            )
    ...
    if not is_valid:
        ...
        return MpesaDepositResponse(
            checkout_request_id="", status="error", message=phone_or_error
        )
    ...
    try:
        ...
        logger.info(
            f"STK Push response: Code={response_code}, CheckoutID={checkout_id}"
        )
        ...
        if response_code == MpesaResultCode.SUCCESS:
            ...
            return MpesaDepositResponse(
                ...
                or response_desc
                or f"Request failed: {response_code}",
            )
        ...
        return MpesaDepositResponse(
            checkout_request_id="", status="error", message=error_message
        )

    except Exception as e:
        logger.error(f"STK Push exception: {str(e)}")
        return MpesaDepositResponse(
            checkout_request_id="", status="error", message=f"Request failed: {str(e)}"
        )
...
def query_stk_status(checkout_request_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"STK Query HTTP error: {e.response.status_code} - {e.response.text}"
        )
        return {"status": "error", "message": f"Query failed: {e.response.status_code}"}
...
def process_mpesa_callback(
    callback_data: Dict[str, Any], db: Session
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if result_code == 0:  # Success
            ...
            transaction = (
                ...
                .filter(Transaction.provider_reference == checkout_request_id)
                .first()
            )
            ...
            if not transaction:
                ...
                if user:
                    ...
                    portfolio = (
                        db.query(Portfolio).filter(Portfolio.user_id == user.id).first()
                    )
                    if portfolio:
                    ...
                    db.commit()
                    logger.info(
                        f"Transaction completed: User={user.email}, Amount={amount}, Account={account_id}"
                    )
                else:
                    logger.warning(f"User not found for phone: {phone_number}")
                    return {"ResultCode": 1, "ResultDesc": "User not found"}
            else:
                ...
                if transaction.account_id:
                    ...
                    if account:
                        account.balance = float(account.balance) + float(
                            transaction.amount
                        )
                        account.total_deposits = float(account.total_deposits) + float(
                            transaction.amount
                        )
                        logger.info(
                            f"Credited broker account {account.id}: {transaction.amount}"
                        )

                # Update portfolio for backward compatibility
                portfolio = (
                    db.query(Portfolio)
                    .filter(Portfolio.user_id == transaction.user_id)
                    .first()
                )
                if portfolio:
                    portfolio.cash = float(portfolio.cash) + float(transaction.amount)
                    portfolio.buying_power = float(portfolio.buying_power) + float(
                        transaction.amount
                    )
                    portfolio.total_value = float(portfolio.total_value) + float(
                        transaction.amount
                    )

                db.commit()
                logger.info(
                    f"Transaction updated: Receipt={mpesa_receipt}, Account={transaction.account_id}"
                )
            ...
            logger.warning(
                f"Transaction failed/cancelled: Code={result_code}, Desc={result_desc}"
            )
...
def initiate_mpesa_withdrawal(
    phone_number: str, amount: float, user_id: str, db: Session
) -> Dict[str, Any]:
    """
    ...
    if not is_valid:
        return {"status": "error", "message": phone_or_error}

    phone_number = phone_or_error

    # Validate amount
    if amount < 10:
        return {"status": "error", "message": "Minimum withdrawal amount is KES 10"}
    ...
    try:
        ...
        logger.info(
            f"B2C response: Code={response_code}, ConversationID={conversation_id}"
        )
        ...
        if response_code == "0":
            ...
            return {"status": "failed", "message": response_desc or "Withdrawal failed"}
...
def process_b2c_result(result_data: Dict[str, Any], db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        transaction = (
            db.query(Transaction)
            .filter(Transaction.provider_reference == conversation_id)
            .first()
        )

        if not transaction:
            logger.warning(
                f"Transaction not found for ConversationID: {conversation_id}"
            )
        ...
        if result_code == 0:  # Success
            ...
            result_parameters = result.get("ResultParameters", {}).get(
                "ResultParameter", []
            )
...
def get_transaction_history(
    user_id: str, db: Session, limit: int = 50, transaction_type: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"transactions": transaction_list, "count": len(transaction_list)}
    except Exception as e:
