                    ...
                    "gainers": [
                        {
                            ...
                            "change_percent": float(item["change_percentage"].replace("%", "")),
...
class FinnhubProvider(MarketDataProvider):
    ...
    def get_historical(self, symbol: str, interval: str = "D", 
                      outputsize: int = 30) -> List[Dict[str, Any]]:
        ...
        try:
            with httpx.Client(timeout=10) as client:
                ...
                result = {
                    ...
                    "change_percent": ((float(quote.get("close", 0)) - float(quote.get("open", 0))) / float(quote.get("open", 1))) * 100 if float(quote.get("open", 0)) > 0 else 0,
...
class ProviderRotator:
    ...
    def get_provider(self) -> MarketDataProvider:
        ...
        for _ in range(len(self.providers)):
            ...
            if provider.is_available():
                ...
                if calls_today < provider_limit:
                    ...
                    logger.info(f"Using {provider_name} ({calls_today + 1}/{provider_limit} calls today)")
                    return provider
                else:
                    logger.warning(f"{provider_name} daily limit reached: {calls_today}/{provider_limit}")
        ...
        raise MarketDataProviderException("All market data providers are currently unavailable or at capacity")
