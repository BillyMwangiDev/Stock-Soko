...
def initiate_mpesa_stk(req: MpesaDepositRequest) -> MpesaDepositResponse:
    ...
    try:
        ...
        return MpesaDepositResponse(
            checkout_request_id="", status="error", message=f"OAuth failed: {str(e)}"
        )
    ...
    try:
        ...
        logger.error(
            f"Daraja STK HTTP error: {e.response.status_code} {e.response.text}"
        )
        return MpesaDepositResponse(
            checkout_request_id="",
            status="error",
...
def process_mpesa_callback(
    ...
    try:
        ...
        logger.info(
            f"Processing callback for {checkout_request_id}: Code {result_code}"
        )
        ...
        if result_code == 0:  # Success
            ...
            if not transaction:
                ...
                if user:
                    ...
                    db.commit()
                    logger.info(
                        f"Transaction completed: {mpesa_receipt}, Amount: {amount}"
                    )
            else:
