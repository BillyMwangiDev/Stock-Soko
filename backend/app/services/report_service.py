...
from ..database.models import Transaction, User, Holding, Stock, Order
...
class ReportService:
    ...
    def generate_transaction_statement(
        ...
        try:
            ...
            elements.append(Paragraph(f"<b>Account Holder:</b> {user.full_name or user.email}", info_style))
            ...
            # Transactions table
            if transactions:
                table_data = [['Date', 'Type', 'Method', 'Amount (KES)', 'Status', 'Reference']]
                ...
                table = Table(table_data, colWidths=[1.2*inch, 0.8*inch, 0.8*inch, 1*inch, 0.8*inch, 1.5*inch])
                ...
                # Summary
                elements.append(Spacer(1, 0.5*inch))
                total_deposits = sum(float(t.amount) for t in transactions if t.type == "deposit" and t.status == "completed")
                total_withdrawals = sum(float(t.amount) for t in transactions if t.type == "withdrawal" and t.status == "completed")
                
                summary_data = [
                    ['Transaction Summary', ''],
                    ['Total Deposits', f"KES {total_deposits:,.2f}"],
                    ['Total Withdrawals', f"KES {total_withdrawals:,.2f}"],
                    ['Net Cash Flow', f"KES {(total_deposits - total_withdrawals):,.2f}"],
                ...
                elements.append(Paragraph("No transactions found for this period.", self.styles['Normal']))
            ...
            logger.info(f"Generated transaction statement for user {user_id}: {len(transactions)} transactions")
            return buffer
    ...
    def generate_tax_report(
        ...
        try:
            ...
            for order in sell_orders:
                ...
                quantity = float(order.filled_quantity) if order.filled_quantity > 0 else float(order.quantity)
                ...
                tax_data.append({
                    'Date': order.submitted_at.strftime('%Y-%m-%d') if order.submitted_at else '',
            ...
            for holding in holdings:
                ...
                if stock and stock.dividend_yield:
                    market_value = float(holding.quantity) * float(stock.latest_price or 0)
            ...
            summary_df = pd.DataFrame([
                {'Date': '', 'Stock Symbol': '', 'Stock Name': '', 'Quantity': '', 'Sale Price': '', 
                 'Cost Basis': '', 'Proceeds': '', 'Cost': '', 'Fees': '', 'Capital Gain/Loss': '', 'Status': ''},
                {'Date': 'SUMMARY', 'Stock Symbol': '', 'Stock Name': '', 'Quantity': '', 'Sale Price': '',
                 'Cost Basis': '', 'Proceeds': '', 'Cost': '', 'Fees': '', 'Capital Gain/Loss': '', 'Status': ''},
                {'Date': 'Total Capital Gains/Losses', 'Stock Symbol': '', 'Stock Name': '', 'Quantity': '',
                 'Sale Price': '', 'Cost Basis': '', 'Proceeds': '', 'Cost': '', 'Fees': '',
                 'Capital Gain/Loss': f'{total_capital_gains:.2f}', 'Status': ''},
                {'Date': 'Estimated Dividend Income', 'Stock Symbol': '', 'Stock Name': '', 'Quantity': '',
                 'Sale Price': '', 'Cost Basis': '', 'Proceeds': '', 'Cost': '', 'Fees': '',
                 'Capital Gain/Loss': f'{total_dividends:.2f}', 'Status': ''},
                {'Date': 'Total Taxable Income', 'Stock Symbol': '', 'Stock Name': '', 'Quantity': '',
                 'Sale Price': '', 'Cost Basis': '', 'Proceeds': '', 'Cost': '', 'Fees': '',
                 'Capital Gain/Loss': f'{(total_capital_gains + total_dividends):.2f}', 'Status': ''},
            ])
            ...
            logger.info(f"Generated tax report for user {user_id}: {len(sell_orders)} transactions")
            return buffer
    ...
    def generate_portfolio_summary(
        ...
        try:
            ...
            elements.append(Paragraph("Stock Soko", title_style))
            elements.append(Paragraph("Monthly Portfolio Summary", self.styles['Heading2']))
            elements.append(Spacer(1, 0.3*inch))
            ...
            elements.append(Paragraph(f"<b>Account:</b> {user.full_name or user.email}", self.styles['Normal']))
            elements.append(Paragraph(f"<b>Period:</b> {month_names[month]} {year}", self.styles['Normal']))
            elements.append(Paragraph(
            ...
            # Holdings table
            if holdings:
                table_data = [['Stock', 'Quantity', 'Avg Price', 'Current Price', 'Market Value', 'P/L', 'P/L %']]
                ...
                table.setStyle(TableStyle([
                    ...
                    ('FONTSIZE', (0, 0), (-1, 0), 10),
                    ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
                    ('BACKGROUND', (0, -1), (-1, -1), colors.HexColor('#2B3139')),
