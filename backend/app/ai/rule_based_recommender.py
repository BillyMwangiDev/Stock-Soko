...
class RuleBasedRecommender:
    ...
    def get_recommendation(self, stock_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        action_points = self._generate_action_points(
            recommendation, stock_data, overall_score
        )
    ...
    def _generate_reasoning(
        ...
        if change > 2:
            reasoning_parts.append(f"Strong upward momentum with +{change:.2f}% gain.")
        elif change < -2:
            reasoning_parts.append(
        ...
        if pe > 0 and pe < 10:
            reasoning_parts.append(f"Trading at attractive P/E ratio of {pe:.1f}x.")
        elif pe > 25:
            reasoning_parts.append(
        ...
        if risk <= 3:
            ...
            reasoning_parts.append(
                "Higher risk level - suitable for aggressive investors only."
            )
    ...
    def _generate_action_points(
        self, recommendation: Recommendation, stock: Dict[str, Any], score: float
    ) -> List[str]:
