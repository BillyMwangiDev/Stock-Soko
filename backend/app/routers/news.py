...
from ..schemas.news import NewsFilter, NewsResponse
...
async def get_news(
    category: Optional[str] = Query(
        None, description="Filter by category or stock symbol"
    ),
    ...
    try:
        result = news_service.get_news(
            category=category, sentiment=sentiment, limit=limit, offset=offset
        )
        return result
...
async def get_live_news(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch live news: {str(e)}"
        )


@router.get("/stock/{symbol}")
async def get_stock_news(
    symbol: str, limit: int = Query(10, ge=1, le=50, description="Number of articles")
):
...
async def get_fundamentals(symbol: str):
    ...
    try:
        ...
        if not fundamentals:
            ...
            return {"symbol": symbol, "data": {}, "source": "unavailable"}

        return {"symbol": symbol, "data": fundamentals, "source": "finnhub"}
    except Exception as e:
        raise HTTPException(
