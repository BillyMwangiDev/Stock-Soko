...
from ..schemas.settings import SecuritySettings, UserPreferences, UserSettings
...
async def get_settings(current_user: dict = Depends(get_current_user)):
    ...
    if user_email not in user_settings_db:
        return UserSettings(preferences=UserPreferences(), security=SecuritySettings())
...
async def update_settings(
    ...
    if settings.security.session_timeout_minutes < 5:
        raise HTTPException(
            status_code=400, detail="Session timeout must be at least 5 minutes"
        )

    if settings.security.session_timeout_minutes > 1440:
        raise HTTPException(
            status_code=400, detail="Session timeout cannot exceed 24 hours"
        )
...
async def update_preferences(
    ...
    if user_email not in user_settings_db:
        user_settings_db[user_email] = UserSettings(
            preferences=preferences, security=SecuritySettings()
        )
    else:
...
async def update_security(
    security: SecuritySettings, current_user: dict = Depends(get_current_user)
):
    """
    ...
    if security.session_timeout_minutes < 5 or security.session_timeout_minutes > 1440:
        raise HTTPException(
            status_code=400, detail="Session timeout must be between 5 and 1440 minutes"
        )

    # Get existing settings or create new
    if user_email not in user_settings_db:
        user_settings_db[user_email] = UserSettings(
            preferences=UserPreferences(), security=security
        )
    else:
...
async def reset_settings(current_user: dict = Depends(get_current_user)):
    ...
    default_settings = UserSettings(
        preferences=UserPreferences(), security=SecuritySettings()
    )
