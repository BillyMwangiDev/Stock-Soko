...
def post_recommendation(req: QuoteRequest) -> dict:
	quote = post_quote(req)
	signal = sma_crossover_signal(quote.sparkline)
	return {"symbol": req.symbol, "recommendation": signal}


@router.post("/indicators", response_model=dict)
def post_indicators(req: QuoteRequest) -> dict:
	quote = post_quote(req)
...
async def get_stock_detail(symbol: str):
	...
	- **symbol**: Stock symbol (e.g., NSE:SCOM)
	
	Returns comprehensive stock data including fundamentals
	"""
...
async def get_sector_performance():
	...
	Returns average performance for each sector
	"""
