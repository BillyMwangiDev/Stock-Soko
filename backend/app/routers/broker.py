...
from ..database.models import User, Account, Broker
...
async def get_user_broker_accounts(
    ...
    try:
        ...
        # Get all accounts
        accounts = db.query(Account).join(Broker).filter(
        ...
        for account in accounts:
            ...
            result.append(BrokerAccountBalance(
                ...
                total_deposits=float(account.total_deposits) if account.total_deposits else 0.0,
                total_withdrawals=float(account.total_withdrawals) if account.total_withdrawals else 0.0,
...
async def get_broker_account_balance(
    ...
    try:
        ...
        # Get account
        account = db.query(Account).join(Broker).filter(
