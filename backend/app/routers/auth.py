...
def register(payload: UserCreate) -> UserPublic:
    ...
    try:
        ...
        user = create_user(
            payload.email, truncated_password, payload.full_name, payload.phone
        )
        return UserPublic(
            email=user.email,
...
async def forgot_password(
    ...
    return {"message": "Password reset link sent to email", "email": email}


@router.post("/reset-password")
async def reset_password(
    req: PasswordReset, db: Session = Depends(get_db)
) -> Dict[str, str]:
    """
    ...
    if email not in password_reset_tokens:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No reset request found"
        )
...
async def verify_2fa_endpoint(
    code: str, email: str = Depends(current_user_email), db: Session = Depends(get_db)
):
    """Verify 2FA code and enable 2FA"""
    from ..database.models import User
    from ..services.twofa_service import enable_2fa, verify_2fa_code
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"2FA verification failed: {str(e)}"
        )
...
async def get_2fa_status(
    ...
    try:
        ...
        return {"email": email, "twofa_enabled": enabled}
...
@router.get("/sessions")
async def get_active_sessions(email: str = Depends(current_user_email)):
...
async def logout_all_sessions(email: str = Depends(current_user_email)):
