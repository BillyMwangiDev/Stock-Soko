...
def validate_password(password: str) -> str:
    ...
    if len(password.encode('utf-8')) > BCRYPT_MAX_PASSWORD_LENGTH:
        raise WeakPasswordException(f"Password too long (max {BCRYPT_MAX_PASSWORD_LENGTH} bytes)")
...
def validate_trade_amount(amount: float) -> float:
    ...
    if amount < MIN_TRADE_AMOUNT:
        raise ValidationException("amount", f"Amount must be at least {MIN_TRADE_AMOUNT}")
