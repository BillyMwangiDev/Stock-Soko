...
def download_production_certificate():
    ...
    print(
        "3. Download 'Production Certificate' (SandboxCertificate.cer or ProductionCertificate.cer)"
    )
...
def interactive_mode():
    ...
    if choice == "1":
        ...
        cert_path = input(
            "Enter path to production certificate [./ProductionCertificate.cer]: "
        ).strip()
        if not cert_path:
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ["-h", "--help"]:
            ...
            print("\n  Command-line mode:")
            print(
                "    python -m backend.app.utils.mpesa_credential_generator <password> [cert_path]"
            )
            print("\nExamples:")
            print("  # Sandbox")
            print(
                "  python -m backend.app.utils.mpesa_credential_generator 'Safaricom999!*!'"
            )
            print("\n  # Production")
            print(
                "  python -m backend.app.utils.mpesa_credential_generator 'MySecretPass' './ProductionCertificate.cer'"
            )
            sys.exit(0)
